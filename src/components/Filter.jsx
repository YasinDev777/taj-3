...
import {
  FilterAnalaysisAnalytics,
  FilterAnalaysisTypeAnalytics,
  FilterClearAnalytics,
  FilterGridAnalytics,
  FilterTimeFrameAnalytics,
...
const Filter = ({
  ...
  useEffect(() => {
    if (forFilterData) {
      ...
      // =======
      const foundPresetData = [
        ...forFilterData.filter((item) => !item.data.is_locked),
        ...forFilterData.filter((item) => item.data.is_locked),
      ].find((item) => item.data.name === selectedPreset);
      // >>>>>>> dev
      if (foundPresetData) {
      ...
      const foundTimeData = forTimeData.find(
        (item) => item.addAnalsisAndTimeframe.name === selectedTime
      );
      if (foundTimeData) {
    ...
    <>
      ...
      <div className="flex w-[98%] m-auto mt-5 bg-white justify-between items-center relative p-8 rounded-t-3xl max-lg:block max-sm:p-3">
        <div className="max-lg:w-full max-lg:flex max-lg:justify-between max-lg:items-center">
          <div>
            <h1 className="text-3xl max-xl:text-2xl max-xs:text-sm">
              Texnik analizlar
            </h1>
        ...
        <div className="flex shadow-[0_0_5px_5px_#0000000D] rounded-xl w-4/5 items-center justify-between p-4 max-xl:w-full max-xl:my-4 max-xs:shadow-none max-xs:p-0 max-xs:py-1">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-500 max-md-plus:hidden">
              {selectedPreset}
            </span>
            <div className="relative">
              <div
                className={`dropdown-trigger flex items-center justify-between w-36 p-2 text-gray-700 bg-white border border-gray-300 rounded-md hover:border-gray-400 max-sm:w-24  ${
                  selectValues ? "cursor-pointer" : "cursor-not-allowed"
                }`}
                onClick={() => {
                  setOpen1(!open1);
                  setOpen(false);
              ...
              <div
                className={`dropdown-content absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg ${
                  open1 === false ||
                  selectedPreset === "All" ||
                  selectedPreset === "Type"
                    ? "hidden"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <div
                className={`dropdown-content absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg ${
                  open3 === false ||
                  forFilterTimeData.some((item) => item.length <= 0)
                    ? "hidden"
                    : "block"
                }`}
              >
                ...
                {forFilterTimeData
                  ...
                  .map((item, index) => (
                    <div
                      key={`${item.name}-${index}`}
                      className={`px-2 py-2 border-t flex items-center justify-between text-sm hover:bg-gray-100 cursor-pointer max-sm:p-2 max-sm:px-1 ${
                        item.is_locked
                          ? "text-gray-400 pointer-events-none"
                          : "text-gray-700"
                      }`}
          ...
          <LuFilterX
            className="text-xl hover:text-gray-600 transition-all max-sm:text-base cursor-pointer"
